        ...
        try
        {
            ...
            cx.Execute("""
                ...
                ON CONFLICT(id) DO UPDATE SET payment_identifier = COALESCE(@Pid, payment_identifier),
                                              ...
                                              situacao = CASE WHEN COALESCE(provedor,'') IN ('paygo','controlpay','pgweblib') THEN situacao ELSE @S END,
                                              atualizado_em = @Em
                """,
        ...
        try
        {
            ...
            cx.Execute("""
                UPDATE tef_transacao SET situacao=@S, motivo=@M, payment_identifier=COALESCE(@Pid, payment_identifier),
                 ...
                 WHERE charge_id=@Ch AND COALESCE(provedor,'') NOT IN ('paygo','controlpay','pgweblib')
                """,
