...
public static class TestesPromocoes
{
    ...
    {
        // contexto de autorizacao VAZIO: nenhuma promocao aqui exige 2FA (ver TestesPromoAutorizacao)
        ...
        var (cent, nome) = Promocoes.PrecoEfetivoCent(promos, donut, "Donuts", 2190, quinta, ctx);
        checar(cent == 1450 && nome == "donuts do dia",
            $"QUINTA: ninho com nutella sai por 14,50 (mediu {cent})");
        (cent, _) = Promocoes.PrecoEfetivoCent(promos, donut, "Donuts", 2190, quarta, ctx);
        checar(cent == 2190, "QUARTA: preco cheio — a regra e so de quinta");
        (cent, _) = Promocoes.PrecoEfetivoCent(promos, "outro-produto", "Donuts", 2190, quinta, ctx);
        checar(cent == 2190, "outro produto nao pega o preco do dia");
        (cent, _) = Promocoes.PrecoEfetivoCent(promos, donut, "Donuts", 2190,
            new DateTime(2026, 10, 1, 15, 0, 0), ctx);
        ...
        var pct = Promocoes.Parsear("""
            ...
            """)!;
        (cent, _) = Promocoes.PrecoEfetivoCent(new[] { pct }, "X", "c", 1000, quarta, ctx);
        checar(cent == 900, "10% na quarta aplica (1000 -> 900)");
        (cent, _) = Promocoes.PrecoEfetivoCent(new[] { pct }, "X", "c", 1000, quinta, ctx);
        ...
        var val = Promocoes.Parsear("""
            ...
            """)!;
        (cent, _) = Promocoes.PrecoEfetivoCent(new[] { val }, "Y", "c", 1200, quinta, ctx);
        checar(cent == 700, "valor fixo desconta em centavos (1200 -> 700)");
        (cent, _) = Promocoes.PrecoEfetivoCent(new[] { val }, "Y", "c", 300, quinta, ctx);
        ...
        (cent, _) = Promocoes.PrecoEfetivoCent(new[] { happy }, "Z", "c", 1000,
            new DateTime(2026, 8, 20, 19, 0, 0), ctx);
        checar(cent == 500, "dentro da janela 18-20h aplica");
        (cent, _) = Promocoes.PrecoEfetivoCent(new[] { happy }, "Z", "c", 1000,
            new DateTime(2026, 8, 20, 21, 0, 0), ctx);
        checar(cent == 1000, "fora da janela nao aplica");

        // ── entre duas promocoes vale a MELHOR pro cliente ──────────────────
        (cent, nome) = Promocoes.PrecoEfetivoCent(new[] { pct, val }, "X", "c", 1000, quarta, ctx);
        ...
        var lxpy = Promocoes.Parsear("""
            ...
            """)!;
        (cent, _) = Promocoes.PrecoEfetivoCent(new[] { lxpy }, "X", "c", 1000, quinta, ctx);
        ...
        (cent, _) = Promocoes.PrecoEfetivoCent(
            new[] { Promocoes.Parsear("""{"id":"x"}""")! }, "X", "c", 1000, quinta, ctx);
        ...
        checar(Promocoes.PrecoEfetivoCent(new[] { porDia }, ovo, "Donuts", 1800, quinta, ctx).Cent == 1450
            && Promocoes.PrecoEfetivoCent(new[] { porDia }, brig, "Donuts", 1800, quinta, ctx).Cent == 1800,
        ...
        checar(Promocoes.AvaliarCarrinho(new[] { desc }, new[] { It("A", 2190), It("B", 2190) }, sexta, ctx).TotalCent == 0,
        ...
        checar(Promocoes.PrecoEfetivoCent(new[] { dom }, "A", "Donuts", 1000, domingo, ctx).Cent == 900,
            "dias_semana [0] (JS) liga no DOMINGO (antes nunca ligava)");
        checar(Promocoes.PrecoEfetivoCent(new[] { dom }, "A", "Donuts", 1000, sabado, ctx).Cent == 1000,
        ...
        var av = Promocoes.AvaliarCarrinho(new[] { dez }, new[] { It("A", 2190) }, sexta, ctx);
        checar(av.PromoId == "dez" && av.DescontoCent[0] == 219, "percentual pelo carrinho: 10% de 21,90 = 2,19 na linha");
        av = Promocoes.AvaliarCarrinho(new[] { dez, cinco }, new[] { It("A", 2190), It("B", 2190) }, sexta, ctx);
        ...
        av = Promocoes.AvaliarCarrinho(new[] { dez, dezB }, new[] { It("A", 2190) }, sexta, ctx);
        checar(av.PromoId == "a-dez", "empate de desconto: vence o menor Id (ordinal), estavel");

        // ── leve X pague Y ─────────────────────────────────────────────────
        av = Promocoes.AvaliarCarrinho(new[] { lx }, new[] { It("A", 2190, 2), It("C", 1350), It("D", 800) }, sexta, ctx);
        ...
        av = Promocoes.AvaliarCarrinho(new[] { lx }, new[] { It("A", 2190, 4), It("C", 1350, 2), It("D", 800) }, sexta, ctx);
        ...
        checar(Promocoes.AvaliarCarrinho(new[] { lx }, new[] { It("A", 2190, 2) }, sexta, ctx).TotalCent == 0,
            "leve 3 pague 2 com 2 unidades: nada");
        checar(Promocoes.AvaliarCarrinho(new[] { lx }, new[] { It("A", 2190, 2), new("D", "Bebidas", 800, 500) }, sexta, ctx).TotalCent == 0,
            "fracao (0,5 un) nao conta como unidade do grupo");

        // ── combo ─────────────────────────────────────────────────────────
        av = Promocoes.AvaliarCarrinho(new[] { cbP }, new[] { It("A", 2190, 2), It("D", 800) }, sexta, ctx);
        ...
        av = Promocoes.AvaliarCarrinho(new[] { cbP }, new[] { It("A", 2190, 4), It("D", 800, 2) }, sexta, ctx);
        checar(av.TotalCent == 1360, "combo cabe 2 vezes: desconto dobra");
        checar(Promocoes.AvaliarCarrinho(new[] { cbP }, new[] { It("A", 2190, 2) }, sexta, ctx).TotalCent == 0,
        ...
        av = Promocoes.AvaliarCarrinho(new[] { cbPct }, new[] { It("A", 2190), It("D", 800) }, sexta, ctx);
        ...
        av = Promocoes.AvaliarCarrinho(new[] { P(Cg("qualquer_item")) }, carrinho, sexta, ctx);
        ...
        av = Promocoes.AvaliarCarrinho(new[] { P(Cg("qualquer_item")) }, caro, sexta, ctx);
        ...
        av = Promocoes.AvaliarCarrinho(new[] { P(Cg("lista", ",\"ganha\":[\"B\"]")) }, caro, sexta, ctx);
        ...
        av = Promocoes.AvaliarCarrinho(new[] { dia, P(Cg("qualquer_item")) }, carrinho, sexta, ctx);
        ...
        av = Promocoes.AvaliarCarrinho(new[] { dia, P(Cg("qualquer_item")) }, new[] { It("A", 2190) }, sexta, ctx);
        ...
        checar(Promocoes.AvaliarCarrinho(new[] { lxJanela }, new[] { It("A", 2190, 2) }, new DateTime(2026, 9, 4, 19, 59, 0), ctx).TotalCent == 2190,
            "leve 2 pague 1 dentro da janela (19:59, sexta)");
        checar(Promocoes.AvaliarCarrinho(new[] { lxJanela }, new[] { It("A", 2190, 2) }, new DateTime(2026, 9, 4, 20, 0, 0), ctx).TotalCent == 0,
            "fora da janela (20:00) nao vale");
        checar(Promocoes.AvaliarCarrinho(new[] { lxJanela }, new[] { It("A", 2190, 2) }, new DateTime(2026, 9, 5, 19, 0, 0), ctx).TotalCent == 0,
            "sabado nao vale (dias_semana [5])");

        // ── invariantes ───────────────────────────────────────────────────
        av = Promocoes.AvaliarCarrinho(new[] { cinco }, new[] { It("D", 300) }, sexta, ctx);
        checar(av.TotalCent == 300, "desconto nunca passa do bruto da linha (5,00 off num item de 3,00 = 3,00)");
        checar(Promocoes.AvaliarCarrinho(new[] { lx, cbP, dia, cinco }, Array.Empty<Promocoes.ItemCarrinho>(), sexta, ctx).TotalCent == 0,
            "comanda vazia: nada");
        ...
        av = Promocoes.AvaliarCarrinho(new[] { pctCombo }, new[] { It("COMBO10", 9900, 1, "Combos") }, sexta, ctx, combos);
        ...
        checar(Promocoes.AvaliarCarrinho(new[] { cbComCombo }, new[] { It("COMBO10", 9900, 1, "Combos"), It("D", 800) }, sexta, ctx, combos).TotalCent == 0,
            "promocao de combo que lista um produto-combo e IGNORADA");
        checar(Promocoes.AvaliarCarrinho(new[] { cbComCombo }, new[] { It("COMBO10", 9900, 1, "Combos"), It("D", 800) }, sexta, ctx).TotalCent == 1700,
        ...
        checar(Promocoes.AvaliarCarrinho(new[] { cgCombo }, new[] { It("A", 9900), It("COMBO10", 9900, 1, "Combos") }, sexta, ctx, combos).TotalCent == 0,
            "compre e ganhe: produto-combo nunca e brinde");
        checar(Promocoes.AvaliarCarrinho(new[] { cgCombo }, new[] { It("A", 9900), It("COMBO10", 9900, 1, "Combos") }, sexta, ctx).TotalCent == 9900,
